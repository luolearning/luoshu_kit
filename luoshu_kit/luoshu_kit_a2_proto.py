...
class LuoshuBridge:
    ...
    def _calc_path_consistency(self, aligned_feat: torch.Tensor) -> torch.Tensor:
        ...
        while current_G >= 9:
            ...
            patches = current.view(current.shape[0], 3, block, 3, block).permute(0, 1, 3, 2, 4)
            patch_means = patches.mean(dim=(-1, -2))   # [N, 3, 3]
            patch_vars = patches.var(dim=(-1, -2), unbiased=False)  # [N, 3, 3]

            # 每层要求：块内稳定 + 块间保留路径差异
            within_loss = patch_vars.mean()

            pm = self._normalize_map(patch_means)
            dx = pm[:, 1:, :] - pm[:, :-1, :]
            dy = pm[:, :, 1:] - pm[:, :, :-1]
            between_loss = -(dx.abs().mean() + dy.abs().mean())

            losses.append(within_loss + 0.1 * between_loss)
    ...
    def regularize(self, lambda_a=5.0, lambda_p=1.0):
